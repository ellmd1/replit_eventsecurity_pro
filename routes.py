...
from flask import (
    abort, g, jsonify, redirect, render_template, request, 
...
from models import (
    ActivityLog, AssessmentTemplate, EventReport,
    SecurityDecision, SecurityInsight, RiskAssessment, ChatMessage
)
...
def register_routes(app):
    ...
    def start_activity_tracking(activity_type, event_report_id=None):
        try:
            ...
            logger.error(f"Error in start_activity_tracking: {str(e)}", exc_info=True)
            return None
    ...
    def end_activity_tracking(log):
        try:
            ...
            logger.error(f"Error in end_activity_tracking: {str(e)}", exc_info=True)
            return log
